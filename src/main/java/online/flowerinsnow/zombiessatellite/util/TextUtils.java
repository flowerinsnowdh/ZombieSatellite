...
public abstract class TextUtils {
    ...
    public static char getColourOfHealth(List<String> patterns, float health) {
        for (String pattern : patterns) {
            int splitIndex;
            if ((splitIndex = pattern.indexOf(':')) == -1 || pattern.length() < splitIndex + 2) {
                continue;
            }
            ...
            if (pattern.startsWith("==")) {
                ...
                int value = Integer.parseInt(pattern.substring(1, splitIndex));
                if (health < value) {
                    return colourCode;
                }
            } else if (pattern.startsWith(">")) {
    ...
    public static boolean isInZombies() {
        ...
        return sidebar != null && ("REPLAY".equals(removeColourCode(sidebar.getDisplayName())) || "ZOMBIES".equals(removeColourCode(sidebar.getDisplayName())));
    }
